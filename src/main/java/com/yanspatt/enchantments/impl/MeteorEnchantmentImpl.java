...
public class MeteorEnchantmentImpl extends CustomEnchantment {

    @Override
    public PickaxeEnchantment type() {
        return PickaxeEnchantment.METEOR;
    }
    ...
    public void blockBreak(User user, BlockHandler handler) {
        ...
        if (randomNumber <= ((type().getChancePerLevel() * user.getPickaxe().getEnchantLevel(type())) * 100)) {
            handler.getPlayer().sendMessage(Component.text("METEORO!!!").color(NamedTextColor.BLACK));
        }
    }
}
